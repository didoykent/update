...
$redis->publish('userconnected', $authuser->id);
     ...
     array_push($latestMassage, $latestMassageLists);
         ...
         $friendLatestMessages =  Message::where('tutors_id', $authuser->id)->where('student_id',$friendLatestMessage[$i]['id'] )->orderBy('created_at', 'DESC')->first();
     array_push($latestMessages, $friendLatestMessages);
       ...
       array_push($allUnread, $messages);
            array_push($latestMassage, $latestMassageLists);
                ...
                $friendLatestMessages =  Message::where('student_id', $authuser->id)->where('tutors_id',$friendLatestMessage[$i]['id'] )->orderBy('created_at', 'DESC')->first();
            array_push($latestMessages, $friendLatestMessages);
...
$id = $request->id;
if($additionalData === $request->additionalData){


  $disconnectUser = Mega::find($id);

  $disconnectUser->isActive = false;
  $disconnectUser->save();
